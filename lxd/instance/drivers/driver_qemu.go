...
func (d *qemu) busAllocatePCIeHotplug(deviceName string, _ bool) (cleanup revert.Hook, busName string, busAddress string, multifunction bool, err error) {
	...
	for eDevName := range d.expandedDevices {
		...
		if eDevName == deviceName {
			...
			return nil, busName, firstFunctionAddress, false, nil
		}
	...
	for i := qemuPCIDeviceIDStart; i < pciSlotCount; i++ {
		...
		return reverter.Clone().Fail, busName, firstFunctionAddress, false, nil
	}
...
func (d *qemu) deviceAttachPCI(deviceName string, pciConfig []deviceConfig.RunConfigItem) error {
	...
	monHook, err := d.addNetDevConfig(qemuBus, d.busAllocatePCIeHotplug, nil, pciConfig)
	if err != nil {
		return err
	}
...
func (d *qemu) generateQemuConfigFile(cpuInfo *cpuTopology, mountInfo *storagePools.MountInfo, busName string, vsockFD int, devConfs []*deviceConfig.RunConfig, fdFiles *[]*os.File) (string, []monitorHook, error) {
	...
	for _, runConf := range devConfs {
		...
		// Add PCI device.
		if len(runConf.PCIDevice) > 0 {
			monHook, err := d.addPCIDevConfig(&cfg, bus.name, busAllocate, runConf.PCIDevice)
			if err != nil {
				return "", nil, err
			}
...
func (d *qemu) addNetDevConfig(busName string, busAllocate busAllocator, bootIndexes map[string]int, nicConfig []deviceConfig.RunConfigItem) (monitorHook, error) {
	...
	if slices.Contains([]string{"pcie", "pci"}, busName) {
		...
		var multi bool
		var err error
		cleanup, devBus, devAddr, multi, err = busAllocate(devName, false)
		if err != nil {
...
func (d *qemu) addPCIDevConfig(cfg *[]cfgSection, busName string, busAllocate busAllocator, pciConfig []deviceConfig.RunConfigItem) (monitorHook, error) {
